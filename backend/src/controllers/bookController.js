...
const issueBook = async (req, res) => {
  try {
    const { book_id, student_email } = req.body;
    const student_id = req.user.id; // ✅ student ID from token

    // check availability
    const [bookRows] = await db.query("SELECT title, quantity FROM books WHERE id = ?", [book_id]);
    if (!bookRows.length || bookRows[0].quantity <= 0) {
    ...
    if (student_email) {
      await sendEmail(
        student_email,
        "Library Book Issued",
        `You have issued "${bookRows[0].title}". Please return it within 7 days.`
      );
    }
